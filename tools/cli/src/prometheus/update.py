...
def update_app(start_path: str | Path | None = None) -> UpdateSummary:
    ...
    if context.core_path and (context.core_path / ".git").exists():
        core_before = _run_git(["rev-parse", "HEAD"], cwd=context.core_path, check=False) or "unknown"
    ...
    core_after = None
    if context.core_path and (context.core_path / ".git").exists():
        core_after = _run_git(["rev-parse", "HEAD"], cwd=context.core_path, check=False) or "unknown"
